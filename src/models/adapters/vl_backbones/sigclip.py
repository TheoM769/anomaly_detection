...
class SigCLIPBackbone(VLBackbone):
    def __init__(self, model_name):
        super().__init__(model_name)
        self.model.eval()

    def load_model(self):
        if self.model_name == "so400m":
            model = AutoModel.from_pretrained("google/siglip2-so400m-patch14-384").eval()
            self.transform = AutoProcessor.from_pretrained("google/siglip2-so400m-patch14-384")
        elif self.model_name == "base":
            model = AutoModel.from_pretrained("google/siglip2-base-patch16-224").eval()
            self.transform = AutoProcessor.from_pretrained("google/siglip2-base-patch16-224")
        else:
    ...
    def extract_features(self, features_processed):
        self.model.eval()
        with torch.no_grad():
            features = self.model(**features_processed)
